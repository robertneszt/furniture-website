<header id="masthead" class="site-header header-four">
	<div class="top-header">
		...
		<?php if( !get_theme_mod( 'disable_mobile_top_header', false ) ){ ?>
			<?php if( ( !get_theme_mod( 'disable_contact_detail', false ) && !get_theme_mod( 'disable_mobile_contact_details', false ) && ( !empty(get_theme_mod( 'contact_phone', '' ) ) || !empty(get_theme_mod( 'contact_email', '' ) ) || !empty(get_theme_mod( 'contact_address', '' ) ) ) ) || ( has_nav_menu( 'menu-3' ) && !get_theme_mod( 'disable_secondary_menu', false ) ) || ( !get_theme_mod( 'disable_header_social_links', false ) & !get_theme_mod( 'disable_mobile_social_icons_header', false ) && bosa_has_social() ) || is_active_sidebar( 'menu-sidebar' ) ){ ?>
			    	        ...
			    	        <div class="header-search-form">
			    	            <form role="search" method="get" action="<?php echo esc_url(home_url('/')); ?>">
			    	                <select class="header-search-select" name="product_cat">
			    	                    <option value=""><?php esc_html_e('All Categories', 'bosa-store'); ?></option> 
			    	                    <?php
			    	                ...
			    	                <input class="header-search-input" name="s" type="text" placeholder="<?php esc_attr_e('Search products...', 'bosa-store'); ?>"/>
				...
				<div class="col-md-3 d-none d-md-block">
					<?php if ( class_exists('WooCommerce' ) ) { ?>
					    <div class="header-right hidden-xs" >
					        <?php
					        if( !get_theme_mod( 'disable_woocommerce_compare', false ) ){ bosa_store_head_compare();
					        }
					        if( !get_theme_mod( 'disable_woocommerce_wishlist', false ) ){ bosa_store_head_wishlist();
					        }
					        if( !get_theme_mod( 'disable_woocommerce_account', false ) ){ bosa_store_my_account();
					        }
					        if( !get_theme_mod( 'disable_woocommerce_cart', false ) ){ bosa_store_header_cart();
					        }
					        ?>
					    </div>	
					<?php } ?>
			...
			<div class="row align-items-center">
				<div class="col-lg-9 d-none d-lg-block">
					<nav id="site-navigation" class="main-navigation d-none d-lg-flex">
						<button class="menu-toggle" aria-controls="primary-menu" aria-expanded="false"><?php esc_html_e( 'Primary Menu', 'bosa-store' ); ?></button>
